...
async def api_probert_inference(request: probertInferenceRequest):
    ...
    Args:
        request: probertInferenceRequest containing protein sequence
        
    Returns:
        JSON: ProtBERT inference results
    """
    try:
        print(request.prot_seq)
        results = predict_flurescent(request.prot_seq)
        return results
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
